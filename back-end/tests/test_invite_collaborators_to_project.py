...
def test_add_collaborator_to_project(client, mock_firestore):
    ...
    mock_project_ref.set = MagicMock()
    ...
    # Verify set was called with correct data (projects.py uses set with merge=True, not update)
    mock_project_ref.set.assert_called_once()
    update_call_args = mock_project_ref.set.call_args[0][0]
